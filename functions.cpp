...
//Done - call al_install_audio, al_init_acodec_addon, and al_reserve_samples
//return true if everything installs correctly, false otherwise
bool initSoundSubsystem(int numSamplesToReserve)
{
	if (!al_install_audio())
	{
		cout << "ERROR - Audio not installed\n";
		return false;
	}
	if (!al_init_acodec_addon())
	{
		cout << "ERROR - Acodec not initted\n";
		return false;
	}
	if (!al_reserve_samples(numSamplesToReserve))
	{
...
bool initAllSubsystems()
{
	...
	//Done - call initSoundSubsystem reserving 1 sample
...
//Done - call al_load_bitmap to open the given file.
...
//Done - call al_create_bitmap with the given width and height
...
//Done - call al_play_sample - set the correct ALLEGRO_PLAYMODE based on the loop bool
// 
...
//Done - call al_draw_scaled_bitmap - make sure you save the old target, set the new target, call al_draw_scaled_bitmap,
...
void blit(ALLEGRO_BITMAP* dest, ALLEGRO_BITMAP* src, const Vector2D& destLoc, const Vector2D& srcLoc, const Vector2D& srcSize, const Vector2D& scale)
{
	...
	al_draw_scaled_bitmap(
		src, 
		srcLoc.getX(), srcLoc.getY(),
		srcSize.getX(), srcSize.getY(),
		destLoc.getX(), destLoc.getY(),
		srcSize.getX() * scale.getX(), srcSize.getY() * scale.getY(),
		0);

	al_set_target_bitmap(pOldTarget);
}

//Done - call blit to copy the entire src bitmap starting at loc in dest bitmap
...
//Done - call al_draw_circle or al_draw_filled_circle based on the filled bool parameter
...
void drawCircle(ALLEGRO_BITMAP* dest, const Vector2D& center, float radius, const ALLEGRO_COLOR& color, bool filled)
{
	...
	if (!filled) 
	{
		al_draw_circle(center.getX(), center.getY(), radius, color, 3 /*thickness*/);
	}
	else
	{
...
//Done - call al_get_bitmap_width and al_get_bitmap_height to return the size of a given bitmap
Vector2D getSize(ALLEGRO_BITMAP* bitmap)
{
	return Vector2D(al_get_bitmap_width(bitmap), al_get_bitmap_height(bitmap));
}

//Done - call al_save_bitmap
void save(ALLEGRO_BITMAP* bitmap, const std::string filename)
{
	al_save_bitmap(filename.c_str(), bitmap);
}

//Done - call al_destroy_bitmap
